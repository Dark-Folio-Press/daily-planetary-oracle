...
`;
  ...
  app.post('/api/spotify/create-playlist', requireAuth, async (req: any, res) => {
    try {
      ...
      // Helper function to get user's display name with fallbacks
      ...
      // Helper function to format date nicely (e.g., "Sept 16th")
      const formatDateNice = (dateStr: string): string => {
        try {
          ...
          const finalSuffix = day >= 11 && day <= 13 ? 'th' : suffix;
          return `${month} ${day}${finalSuffix}`;
        } catch {
          return dateStr; // fallback to original if parsing fails
        }
      };
      
      // Helper function to safely get rising sign
      const getUserRisingSign = async (user: any): Promise<string | null> => {
        try {
          if (user.birthDate && user.birthTime && user.birthLocation) {
            return astrologyService.calculateRising(user.birthDate, user.birthTime, user.birthLocation);
          }
          return null;
        } catch (error) {
          console.log('Error calculating rising sign:', error);
          return null;
        }
      };
      
      // Generate personalized cosmic playlist name and description
      const generateCosmicPlaylistInfo = async (user: any, aiPlaylist: any): Promise<{name: string, description: string}> => {
        const displayName = getUserDisplayName(user);
        const risingSign = await getUserRisingSign(user);
        const weekStartFormatted = formatDateNice(aiPlaylist.weekStart);
        
        // Generate personalized name
        let cosmicName;
        if (risingSign) {
          cosmicName = `${displayName}'s Cosmic Playlist - ${risingSign} Rising Week of ${weekStartFormatted}`;
        } else {
          cosmicName = `${displayName}'s Cosmic Playlist - Week of ${weekStartFormatted}`;
        }
        
        // Generate mystical description
        const mysticalDescriptions = [
          "Musical sonification of your planetary transits and harmonic frequencies aligned with your birth chart",
          "Celestial harmonies weaving through your cosmic blueprint, translating planetary movements into sonic frequencies",
          "A vibrational symphony channeling your astrological essence into musical form",
          "Sonic interpretation of your stellar influences and cosmic currents flowing through this temporal cycle",
          "Harmonic frequencies attuned to your natal chart's celestial signature"
        ];
        
        const baseDescription = mysticalDescriptions[Math.floor(Math.random() * mysticalDescriptions.length)];
        
        let fullDescription;
        if (risingSign) {
          fullDescription = `${baseDescription}. This week's cosmic soundtrack is specially calibrated for your ${risingSign} rising energy, channeling the universe's frequencies into melodies that resonate with your soul's journey through the stars.`;
        } else {
          fullDescription = `${baseDescription}. This cosmic soundtrack channels universal frequencies into melodies that align with your soul's journey through the celestial realm.`;
        }
      ...
      // Generate personalized cosmic playlist info
      const cosmicPlaylistInfo = await generateCosmicPlaylistInfo(user, aiPlaylist);
      const name = playlistName || cosmicPlaylistInfo.name;
      const description = cosmicPlaylistInfo.description;
      ...
      const html = `
        ...
        <body>
          <div class="container">
            ...
            <a href="${appUrl}" class="cta">🎵 Get Your Free Playlist</a>
          </div>
