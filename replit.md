...
- **Planetary Sound Oracle — Symbolic Layer**: Three-layer colour system per planet (spectral/scientific via Kepler frequency octave transposition, tarot traditional, and astrological), tarot card correspondences pulled from full 936-row CSV (43 matched records across 7 planets), Elder Futhark rune, Hebrew letter/meaning, gemstone, keywords, mythical context. Elemental audio filter presets (Air=Pop Synth, Water=Ambient, Earth=Folk, Fire=Grunge) auto-switch when a planet is selected based on its tarot elemental (Sylph/Undine/Gnome/Salamander). BiquadFilterNode + WaveShaperNode chain inserted into audio graph.
