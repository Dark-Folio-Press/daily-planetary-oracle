...
- **Daily Planetary Oracle (`/daily-planetary-oracle`) — Merged page**: Combines GPT-4o daily forecast (cosmic weather overview, cosmic warning, 4-stat grid: element/best window/avoid/mantra, elemental mood) with the interactive orrery and sound engine. Clicking a planet card plays its Kepler frequency, opens the full symbolic layer (three-colour system: spectral/Kepler, tarot traditional, astrological; Elder Futhark rune; Hebrew letter; gemstone; keywords; mythical context; minor arcana spread), and shows that planet's GPT-4o forecast interpretation and advice — all in one panel. Full Planetary Readings section below shows all 7 expandable planet cards. Dismissible intro panel with "Read more" expand and a persistent "How does this work?" re-show link. Route `/daily-forecast` removed; API endpoint retained server-side.
